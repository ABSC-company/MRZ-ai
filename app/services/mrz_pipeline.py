...
if os.name == "nt":
    from app.utils.turbojpeg_compat import install_turbojpeg_fallback

    install_turbojpeg_fallback()
...
def validate_mrz(texts: list[str]) -> MRZValidation:
    ...
    if is_td3:
        ...
        if l1.startswith("PNRUS") and set(l2[21:28]) == {"<"}:
            l2 = l2[:27] + "0" + l2[28:]
...
def main() -> int:
    ...
    parser.add_argument("--input",      type=Path, default=project_root / "local_samples")
    parser.add_argument("--out",        type=Path, default=Path("pipeline_results"))
    parser.add_argument("--crop-model", type=Path, default=project_root / "weights" / "unet_resnet34.pth")
